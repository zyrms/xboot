...
int mmc_switch(struct mmc *mmc, u8_t set, u8_t index, u8_t value)
{
	struct mmc_cmd cmd;
	int timeout = 1000;
	int ret;
...
int sd_switch(struct mmc *mmc, int mode, int group, u8_t value, u8_t *resp)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
retry_scr:
	...
	err = sd_switch(mmc, SD_SWITCH_SWITCH, 0, 1, (u8_t *)&switch_status);
...
void mmc_set_clock(struct mmc *mmc, u32_t clock)
{
	if (clock > mmc->f_max)
		clock = mmc->f_max;
...
void mmc_set_bus_width(struct mmc *mmc, u32_t width)
{
	mmc->bus_width = width;
...
int mmc_init(struct mmc *mmc)
{
	...
	if (err) {
		...
		if (err) {
			mmcdbg("Card did not respond to voltage select!\n");
			return UNUSABLE_ERR;
		}
	}
