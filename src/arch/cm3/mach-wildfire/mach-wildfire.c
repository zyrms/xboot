 ...
 */
static struct machine wildfire = {
	.info = {
		.board_name 		= "stm32",
		.board_desc 		= "stm32",
		.board_id			= "0x00000000",
...
static __init void mach_wildfire_init(void)
{
	if(!register_machine(&wildfire))
		LOG_E("failed to register machine 'wildfire'");
}

module_init(mach_wildfire_init, LEVEL_MACH);
